...
window.onload = function () {
  ...
  var fetchMoon = function (latitude, longitude) {
    let url =
      'https://api.ipgeolocation.io/astronomy?apiKey=bc821545b212458b93d35ab006e86be3&lat=' +
      latitude +
      '&long=' +
      longitude
    fetch(url, { method: 'GET' })
      ...
      .then(response => {
        ...
        document.getElementById('date_earth').innerHTML =
          ' ' + 'DATE ON EARTH : ' + response.date
        document.getElementById('earth_latitude').innerHTML =
          ' ' + 'LATITUDE : ' + response.location.latitude
        document.getElementById('earth_longitude').innerHTML =
          ' ' + 'LONGITUDE : ' + response.location.longitude
        document.getElementById('moon_azimuth').innerHTML =
          ' ' + 'MOON AZIMUTH : ' + response.moon_azimuth
        document.getElementById('moon_altitude').innerHTML =
          ' ' + 'MOON ALTITUDE  : ' + response.moon_altitude
        document.getElementById('moon_distance').innerHTML =
          ' ' + 'MOON DISTANCE : ' + response.moon_distance
        ...
        document.getElementById('solar_noon').innerHTML =
          ' ' + ' SOLAR NOON : ' + response.solar_noon
        document.getElementById('sun_altitude').innerHTML =
          ' ' + ' SUN ALTITUDE : ' + response.sun_altitude
        document.getElementById('sun_azimuth').innerHTML =
          ' ' + ' SUN AZIMUTH : ' + response.sun_azimuth
        document.getElementById('sun_distance').innerHTML =
          ' ' + 'SUN DISTANCE : ' + response.sun_distance
...
var fetchReverseGeoloc = function (longitude, latitude) {
  let url =
    ...
    '+' +
    longitude +
    '&key=60615941f35440d0a11fd071b32532a0'
  fetch(url, { method: 'GET' }, { mode: 'no-cors' })
    .then(response => response.json())
    .then(response => {
      console.log(response)
      document.getElementById('earth_location').innerHTML =
...
window.addEventListener('moontime:updated', function (e) {
  document.querySelector('.moonClock').innerHTML =
    ' ' + 'HOUR ON MOON : ' + ' ' + moon.formatMoonString('%MS:%MM:%M')
})
...
function randomPoetry () {
  ...
  for (
    moonPoetryRandom;
    moonPoetryRandom < moonPoetry.length;
    moonPoetryRandom++
  ) {
