let moon = {
  ...
  _getLocalTime: () =>
    moon._initialMoon + (performance.now() - moon._initialT) / 4.97875,
  _getSmoothTime: () =>
    moon._smoothMoon + (performance.now() - moon._smoothUpdate) / 4.97875,
  _getAPITime: () =>
    moon._apiMoon === undefined || moon._apiUpdate === undefined
      ? undefined
      : moon._apiMoon + (performance.now() - moon._apiUpdate) / 4.97875,

  _fetchAPI: () => {
    let start = performance.now()
    fetch('https://api.dynodel.com/moon')
      .then(res => res.text())
      .then(ret => {
        moon._apiMoon =
          Number(ret.split(':')[1]) -
          (new Date().getTime() - Number(ret.split(':')[0])) / 4.97875
        moon._apiUpdate = performance.now()
        moon.apiConnected = true

        let event = new CustomEvent('moontime:api_status_changed', {
          detail: moon.apiConnected
        })
  ...
  currentTime: () =>
    moon.settings.smooth_sync ? moon._getSmoothTime() : moon._getAPITime(),
  ...
  _updateSmoothMoon: () => {
    ...
    if (moon.apiConnected) {
      ...
      if (offset > 1000 && moon.settings.jump_large_diffs) {
        ...
        moon._smoothMoon =
          moon._smoothMoon +
          (performance.now() - moon._smoothUpdate) / 4.97875 +
          offset / moon.settings.smooth_factor
        moon._smoothUpdate = performance.now()
      }
    } else {
    ...
    window.setTimeout(
      moon._updateSmoothMoon,
      983.575 - (performance.now() - start) - 10
    )

    let event = new CustomEvent('moontime:updated', {
      detail: moon.currentTime()
    })
    window.dispatchEvent(event)
  },
  ...
  formatMoonString: (s, t = moon.currentTime()) => {
    ...
    formats.forEach(
      format => (string = string.split('%' + format).join(ft[format]))
    )
  ...
  init: (t = 'manual') => {
    ...
    scripts.forEach(script => {
      if (script.getAttribute('src') !== null) {
        if (
          script.getAttribute('src').indexOf('moontime.js') > -1 &&
          script.getAttribute('delay_start') === '' &&
          t === 'auto'
        ) {
          return false
        } else {
          ...
          moon._smoothOffsetUpdate = window.setInterval(() => {
            ...
            if (!isNaN(moon._smoothOffset) && moon.apiConnected) {
              let event = new CustomEvent('moontime:offset_updated', {
                detail: moon._smoothOffset
              })
              window.dispatchEvent(event)
            }
          }, 500)
